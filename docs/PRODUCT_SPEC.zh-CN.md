...
- 能力合同不可用或使用内置 fallback 时隐藏“一键去背景”；可用时在原图/拼豆对比控件附近显示次级操作“一键去背景”。不得常驻显示去背景辅助说明；只有处理中、成功或失败时才在独立的 `aria-live="polite"` 状态区显示上下文状态，空状态不得占据工具栏高度。
...
确认页必须显示检测行列、置信度状态、后端 warning（如有）和网格覆盖层。完整合同且
没有 warning 时显示“网格置信度：高”；完整合同带 warning 时显示“网格置信度：
需要确认”，用户必须在看到 warning 后再次明确确认；无有效合同或合同校验失败时显示
“网格置信度：不足”，禁止镜像，只允许调整边界、修改行列数或重新识别。不得新增未经
样例校准的前端数值阈值。
